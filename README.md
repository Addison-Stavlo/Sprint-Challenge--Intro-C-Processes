...
1. List all of the main states a process may be in at any point in time on a standard Unix system. Briefly explain what each of these states means.
   ...
   Waiting - process is waiting for a resource or event to occur
   Stopped - process has been stopped
   Zombie - the process is dead but still on the table.

2. What is a zombie process?
   its a dead process that is waiting to be removed from the process table.

3. How does a zombie process get created? How does one get destroyed?
   once a process has completed its execution it is set to a 'terminated state'. It is a zombie while it remains on the system table until it is removed. Often child processes sit in zombie state until their parent process is able to read their exit status. Once this is complete, the zombie process can be terminated completely and removed from the table.

4. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?
   We have more control over what we are doing and more access to the underlying systems. We compile our code to machine code, where as other languages just interperet the code to the machine. Compiled code does not need to be translated live and thus is much faster and more efficient.
